...
<script lang="ts">
  ...
  import { MediaHandler, type MediaForm, type MediaFormCollection, type MediaProp } from '$lib/domain/media';
  ...
  let { label = '', name, required = false, multiple = false, ...rest }: {
    label?: string;
    name: MediaFormCollection;
    required?: boolean;
    multiple?: boolean;
    [key: string]: any;
  } = $props();
  ...
  let existingFiles: MediaProp[] = $state([]);

  $effect(() => {
    const prop = rest[name];
    if (prop) {
      existingFiles = Array.isArray(prop) ? prop : [prop];
    }
  });
  ...
  {#each existingFiles as existing}
    <div class="form-control w-full mb-4">
      ...
      <div class="flex items-center gap-4 justify-start">
        ...
        <DownloadLink href={route('media.download', { media: existing.id })} filename={existing.file_name}>
          Download
        </DownloadLink>
        <DeleteButton
          label="Delete"
